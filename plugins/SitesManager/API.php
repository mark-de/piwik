...
class Piwik_SitesManager_API 
{
	...
	{
		Piwik::checkUserIsSuperUser();
		// @TODO Ancud-IT GmbH fix filter to catch backticked part of string
		$quotes = Piwik_Common::isOracle() ? '"' : '`';  
		$groups = Zend_Registry::get('db')->fetchAll("SELECT DISTINCT " . $quotes .  "group" . $quotes. " FROM ".Piwik_Common::prefixTable("site"));
		$cleanedGroups = array();
	...
	public function getSitesIdWithVisits($timestamp = false)
	{
		...
		//@TODO Ancud-IT GmbH	FROM-part in $subQuery obviously needs piwik_site-table too!
        $time = Piwik_Date::factory((int)$timestamp)->getDatetime();
		
		$subQuery = " SELECT 1 "
                ."FROM " . Piwik_Common::prefixTable('log_visit') . " v, " . Piwik_Common::prefixTable('site') . " s "
                ."WHERE v.idsite = s.idsite "
                ."AND visit_last_action_time > ? "
                ."AND visit_last_action_time <= ?  ";
		// Ancud-IT GmbH moved limit to Zendfunction
		$subQuery = Zend_Registry::get('db')->limit($subQuery, 1);
		...
		$result = Piwik_FetchAll("
            ...
            WHERE EXISTS ( ". $subQuery . ")", array($time, $now = Piwik_Date::now()->addHour(1)->getDatetime()));
	...
	public function addSite( $siteName, $urls, $ecommerce = null, $siteSearch = null, $searchKeywordParameters = null, $searchCategoryParameters = null, $excludedIps = null, $excludedQueryParameters = null, $timezone = null, $currency = null, $group = null, $startDate = null )
	{
		...
		// @TODO Ancud-IT GmbH lastInsertId needs a tablename as argument
		// Oracle uses sequences for autogenerated ids
		$idSite = Piwik_Common::isOracle() ?
				$db->lastInsertId(Piwik_Common::prefixTable("site")) :
				$db->lastInsertId();
	...
	public function getPatternMatchSites($pattern)
	{
		...
		$sql = "SELECT idsite, name, main_url "
				.	"FROM "
				.	Piwik_Common::prefixTable('site') . " s "	
 				.	"WHERE (s.name like ? OR s.main_url like ? "
 				.	$where . ") AND idsite in (" . $ids_str . ") ";
		
		$db = Zend_Registry::getDb();
		$sql = $db->limit($sql, Piwik::getWebsitesCountToDisplay());
